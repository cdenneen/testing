...
def index_bucket = 'proteus-dvx-apcapdevelopment-us-east-1-data'
   ...
   stage('fetch code') {
    ...
    stage('build') {
        ...
        parallel(srs: {
    		...
    		node('unixaws') {
    	        unstash "app-stash"
    	        build job: 'shared/jobs/build/create app facter', parameters: [string(name: 'environment', value: 'dev'), string(name: 'PublicBucket', value: bucket), string(name: 'CF_TEMPLATE_NAME', value: cf_template_name), string(name: 'PropertiesFile',  value: env.WORKSPACE + '/Config/Dev/proteus-dvx-us-east-1.properties'), string(name: 'FileName', value: 'app.txt'), string(name: 'SlackChannel', value: slack_channel)]
    		}    		
        })
    } 

...
def index_bucket = 'proteus-dev-apcapdevelopment-us-east-1-data'
...
try {
   ...
   stage('fetch code') {
        parallel(infracode: {
            node('unixaws') {
                build 'proteus/jobs/source control/infrastructure code master'
                
                dir(env.UNIX_WORKSPACE + '/proteus/jobs/source control/infrastructure code master') {
    ...
    stage('build') {
        ...
        parallel(srs: {
    		...
    		node('unixaws') {
    	        unstash "app-stash"
    	        build job: 'shared/jobs/build/create app facter', parameters: [string(name: 'environment', value: 'dev'), string(name: 'PublicBucket', value: bucket), string(name: 'CF_TEMPLATE_NAME', value: cf_template_name), string(name: 'PropertiesFile',  value: env.WORKSPACE + '/Config/Dev/proteus-dev-us-east-1.properties'), string(name: 'FileName', value: 'app.txt'), string(name: 'SlackChannel', value: slack_channel)]
    		}    		
        })
    } 
    ...
    stage('deploy') {
        node('unixaws') {
            ...
            build job: 'shared/jobs/deploy/unix write properties', parameters: [string(name: 'environment', value: props.EnvironmentType), 
                                                                                string(name: 'slack_channel', value: slack_channel), 
                                                                                string(name: 'filename', value: 'proteusdevstaticstack.properties'), 
                                                                                string(name: 'name', value: cf_static_template_name)]
                                                                                
            dir(env.UNIX_WORKSPACE + '/shared/jobs/deploy/unix write properties') {
                staticprops = readProperties file: 'proteusdevstaticstack.properties'
            } 
            ...
            build job: 'shared/jobs/deploy/get ami', parameters: [string(name: 'environment', value: 'dev'), 
                                                                  ...
                                                                  string(name: 'filename', value: 'dataamidev.properties'), 
                                                                  string(name: 'slack_channel', value: slack_channel)]
        
            dir(env.UNIX_WORKSPACE + '/shared/jobs/deploy/get ami') {
            	dataamidev = readProperties file: 'dataamidev.properties'
            }        
            ...
            build job: 'shared/jobs/deploy/unix write properties', parameters: [string(name: 'environment', value: 'dev'), 
                                                                                string(name: 'slack_channel', value: slack_channel), 
                                                                                string(name: 'filename', value: 'proteusdevstack.properties'), 
                                                                                string(name: 'name', value: cf_template_name)]
                                                                                
            dir(env.UNIX_WORKSPACE + '/shared/jobs/deploy/unix write properties') {
                stackprops = readProperties file: 'proteusdevstack.properties'
            }
    ...
    stage('release') {
        node('unixaws') {
            parallel(deletestack: {
                ...
                build 'proteus/jobs/source control/merge application code'
            })    
        }
    }
